...
if ($_SERVER['REQUEST_METHOD']==='POST') {
  ...
  if ($action==='create') {
    ...
    if ($name!=='') q($pdo,"INSERT INTO topics(name,sort_order,is_active) VALUES(?,?,1)",[$name,$sort]);
  } elseif ($action==='update') {
    $id=(int)($_POST['id']??0);
    $name=trim($_POST['name']??'');
    $sort=(int)($_POST['sort_order']??10);
    $active=isset($_POST['is_active'])?1:0;
    if ($id && $name!=='') q($pdo,"UPDATE topics SET name=?,sort_order=?,is_active=? WHERE id=?",[$name,$sort,$active,$id]);
  } elseif ($action==='delete') {
    $id=(int)($_POST['id']??0);
    if ($id) q($pdo,"DELETE FROM topics WHERE id=?",[$id]);
  }
  header("Location: topics.php"); exit();
}

$list=q($pdo,"SELECT id,name,sort_order,is_active FROM topics ORDER BY sort_order,name")->fetchAll(PDO::FETCH_ASSOC);
?>
...
<div class="card" style="margin-bottom:12px;">
  <form method="post" class="row">
    ...
    <input type="text" name="name" placeholder="New topic name" required>
        ...
        <tr>
          <td style="width:60%">
            <form method="post" class="row">
              ...
              <input type="number" name="sort_order" value="<?=$t['sort_order']?>" min="0" step="5">
